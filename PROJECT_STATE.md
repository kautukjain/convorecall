...
| 11 | **4 of 5 audio files are stale** — rendered with macOS `say`, which reads flatly | Now | `objection-call` (the demo call) is neural and verified. The rest need `pnpm --filter @opengong/generators start <call>` once the key's daily cap resets at 00:00 UTC — roughly 45 requests per day, and a call costs one per segment |
