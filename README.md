**What was said. What backs it up. What’s next.**

Upload a sales call or meeting. Get a diarized transcript and deal notes where every claim points back to the exact line that proves it — or the claim does not ship.
...
```bash
pnpm install && pnpm run setup && pnpm seed && pnpm dev
```
...
Sandbox mint (used by `pnpm setup` or `pnpm run setup` when the key is empty):
...
```bash
pnpm install
pnpm run setup      # .env, sandbox key if empty, Postgres :5433, migrations
pnpm seed          # sample fixtures
pnpm dev           # web :3000 + API :3001
```
...
| OpenAPI    | [http://localhost:3001/api/v1/docs](http://localhost:3001/api/v1/docs)     |
| Postgres   | `localhost:5433` (user/db `convorecall`)                                   |
...
support it will ship. Observed: Recap once returned `{quote: "I think that's everyone", category: "Interest"}` — the seller’s opening line — and it cleared the gate legitimately.
...
| `pnpm setup` OR `pnpm run setup` | `.env`, sandbox key, Postgres, migrations                           |
| `pnpm seed`                      | Sample fixtures                                                     |
| `pnpm dev`                       | Web + API                                                           |
...
| Notes differ from the Recap dashboard | Fixtures cache under `convorecall-<name>`, uploads under the call id — the same audio analysed twice is two runs, and Recap is nondeterministic                               |
...
**Forged in 33 hours. Built to kill a $1,400 / seat giant.**

Made with a lot of caffeine, Claude, Cursor, and zero respect for seat licenses  
by **The Hacksmiths**  
